...
// Find the </div> right before </aside> that closes drawer-body
let bodyEnd = endIdx;
while (bodyEnd > startIdx && content.slice(bodyEnd - 1, bodyEnd) !== '>') {
  bodyEnd--;
}
// Now bodyEnd points to '>' of </div>. We need to include the newline before </aside>
const before = content.slice(0, startIdx);
const after = content.slice(endIdx);

const newBody = `            <div className="crm-presale-product-drawer-body">
              {/* 1. 打板信息（独立身份板块） */}
