...
function shouldIgnoreCommandError(text) {
  return /deprecated|experimental/i.test(text) || /^npm warn\b/i.test(text)
}

function startOrAttachVite() {
  logs.push('Checking Vite server...')
  const npmCommand = getPreferredNpmCommand()
  const spawnSpec = getSpawnCommandSpec(npmCommand, CONFIG.devCommand)
  ...
  child.stderr.on('data', (data) => {
    ...
    if (text) {
      // 过滤掉一些正常的警告信息
      if (!shouldIgnoreCommandError(text)) {
        errors.push(text)
        lastErrorTime = Date.now()
      }
      logs.push(text)
...
async function runCommandCheck({ label, command, args = [], env = {}, logTag }) {
  ...
  return new Promise((resolve) => {
    ...
    const appendLog = (line, isError = false) => {
      ...
      if (isError && !shouldIgnoreCommandError(line)) {
...
async function runBuildCheck(entryKey) {
  ...
  return new Promise((resolve) => {
    ...
    const spawnSpec = getSpawnCommandSpec(npxCommand, ['vite', 'build'])
    
    // 使用 ENTRY_KEY 环境变量触发单独构建
    const buildProcess = spawn(spawnSpec.command, spawnSpec.args, {
      cwd: APP_ROOT,
    ...
    buildProcess.stderr.on('data', (data) => {
      ...
      if (text) {
        ...
        // 捕获构建错误
        if (/error|failed/i.test(text) && !shouldIgnoreCommandError(text)) {
