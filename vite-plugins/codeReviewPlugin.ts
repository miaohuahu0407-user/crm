...
function buildListPreview(
  ...
  let rawExpression: ts.Expression | undefined;
  if (ts.isPropertyAssignment(property)) {
    rawExpression = property.initializer;
  } else if (ts.isShorthandPropertyAssignment(property)) {
    rawExpression = property.name;
  }

  if (!rawExpression) {
    return {
      sourceKey: null,
      raw: property.getText(sourceFile),
      items: [],
      parseStatus: 'raw',
      warnings: normalizeWarnings([`字段 ${key} 无法静态解析`]),
    };
  }
